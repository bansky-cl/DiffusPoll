...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_plm_init', type=str, default='no', choices=['no', 'bert','t5','gpt'], help='load init parameter from the pre-trained lm')
    ...
    ###  add mask
    parser.add_argument('--add_token', type=int, default=0, help='add token in tokenizer')
    parser.add_argument('--self_conditions', type=int, default=0, help='whether use self_conditions tricks')
    parser.add_argument('--mask', type=int, default=0, help='add task-specific mask strategy')
    parser.add_argument('--mix', type=int, default=0, help='mix fp16 and fp32')
    parser.add_argument('--use_en_de', type=int, default=0, help='use T5 encoder-decoder net arch')
    parser.add_argument('--div_loss', type=int, default=0, help='train use nll and div_loss')
    ...
    COMMANDLINE = f"OPENAI_LOGDIR={Model_FILE}  " \
                  ...
                  f"--hidden_dim {args.hidden_dim} --self_conditions {args.self_conditions} " \
                  f"--learning_steps {args.learning_steps} --save_interval {args.save_interval} " \
                  f"--config_name {args.config_name} --notes {args.notes} --add_token {args.add_token} --mask {args.mask}" \
