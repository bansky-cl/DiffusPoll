...
if __name__ == '__main__':
    ...
    # parser.add_argument('--add_token', type=int, default=0, help='add token to vocab')
    ...
    for lst in glob.glob(args.model_dir):
        print(lst)
        checkpoints = sorted(glob.glob(f"{lst}/{args.pattern}*.pt"))[::-1] # 获取ckpts 的model .pt 通常只取最后的模型
        print(f"use ckpts {checkpoints[0]}") # 只看最后一个
        ...
        for checkpoint_one in checkpoints:
            ...
            COMMAND = f'export CUDA_VISIBLE_DEVICES={card} && python -m torch.distributed.launch --nproc_per_node=1 --master_port={12233 + card + int(args.seed)} --use_env sample_seq2seq.py ' \
            ...
            f'--cf_w {args.cf_w} --cf_type {args.cf_type} ' ## add cf
