...
from transformers import AutoTokenizer, PreTrainedTokenizerFast, AddedToken, PreTrainedTokenizer,BertTokenizerFast,GPT2TokenizerFast
...
class myTokenizer():
    ...
    def __init__(self, args):
        if args.vocab == 'bert':
            print("### use bert cn tokenizer vocab")
            # tokenizer = AutoTokenizer.from_pretrained(args.config_name)
            # bert-base-chinese
            tokenizer = AutoTokenizer.from_pretrained("bert-base-chinese") # use you own hub
            
            print(f"### add_token set {args.add_token}")
            if args.add_token==1:
                print("### add token")
                for token in ["DIGIT", "<c>"]:
            ...
            if add_star_token==1:       
                special_tokens = []
                with open("Chinese_Star_Corpus.txt", "r") as f_vocab: # you can find this star vocab in open github
                    for line in f_vocab:
                        special_tokens.append(line.strip())
                    f_vocab.close()

                for token in special_tokens:
                    tokenizer.add_tokens(AddedToken(token, single_word=True), special_tokens=True)  
                print("### add star token ",len(tokenizer))
            
            self.tokenizer = tokenizer
            self.worker = 4 # fast tokenizer
        ...
        ### add t5+jieba
            ...
            print("### use T5 pegasus tokenizer vocab")
            tokenizer = T5PegasusTokenizer.from_pretrained("imxly/t5-pegasus") # or use your own hub
            print(f"### add_token set {args.add_token}")
            if args.add_token==1:
                print("### add token")
                for token in ["DIGIT", "<c>"]:
            ...
            if add_star_token==1:       
                special_tokens = []
                with open("Chinese_Star_Corpus.txt", "r") as f_vocab: # 
                    for line in f_vocab:
                        special_tokens.append(line.strip())
                    f_vocab.close()
            ...
            self.sep_token_id = tokenizer.sep_token_id
            self.pad_token_id = tokenizer.pad_token_id
            self.worker = 1 # slow tokenizer
            # save
            tokenizer.save_pretrained(args.checkpoint_path)    
            
        ### add bart
        elif args.vocab == 'bart':
            print("### use BART tokenizer vocab")
            # bart-base-chinese 51271
        ...
        ### add gpt-2
            ...
            print("### use GPT2 tokenizer vocab")
            # bart-base-chinese 51271
            tokenizer = GPT2TokenizerFast.from_pretrained('bart-base-chinese')
        ...
        self.vocab_size = len(self.tokenizer) # updatign 
    ...
    def encode_token(self, sentences):
        ...
        elif isinstance(self.tokenizer, PreTrainedTokenizerFast): # bert 走这里
            input_ids = self.tokenizer(sentences, add_special_tokens=True)['input_ids']
        elif isinstance(self.tokenizer, PreTrainedTokenizer): # t5-pegasus
            input_ids = self.tokenizer(sentences, add_special_tokens=True)['input_ids']
        else:
            assert False, "invalid type of vocab_dict" #T5 这里报错
        return input_ids
    ...
    def decode_token(self, seq):
        ...
        elif isinstance(self.tokenizer, PreTrainedTokenizerFast): # bert
            seq = seq.squeeze(-1).tolist()
            while len(seq)>0 and seq[-1] == self.pad_token_id:
                seq.pop()
            tokens = self.tokenizer.decode(seq)   
        elif isinstance(self.tokenizer, PreTrainedTokenizer): # t5
            seq = seq.squeeze(-1).tolist()
            while len(seq)>0 and seq[-1] == self.pad_token_id:
                seq.pop()
            tokens = self.tokenizer.decode(seq)
...
def create_model_and_diffusion(
    ...
    mask, ### add
    div_loss, ### add loss,
    use_en_de, ### choose encoder or encoder-decoder arch
    **kwargs,
):
    print(f"### in create model fn , self_conditions is {self_conditions}")
    print(f"### in create model fn , net_mask is {mask}")
    model = TransformerNetModel(
        ...
        self_conditions=self_conditions, ### add self_conditions
        mask=mask, ### add net_mask
        use_en_de = use_en_de, ### use decoder
    )
    ...
    diffusion = SpacedDiffusion(
        ...
        self_conditions = self_conditions, ## add self_conditions
        mask=mask, ### add mask
        div_loss = div_loss, ### add div loss
        use_en_de = use_en_de, ### use encoder_decoder
    )
