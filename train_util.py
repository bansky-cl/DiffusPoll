...
class TrainLoop:
    ...
    def run_loop(self):
        ### add tqdm par
        pbar = tqdm(total=self.learning_steps, desc="training")
        while (
            ...
            if self.data_unsup is not None: 
                batch_unsup, cond_unsup = next(self.data_unsup)
            else: 
                batch_unsup = None
                cond_unsup = None
    ...
    def run_step(self, batch, batch_unsup, cond, cond_unsup):
        if np.random.rand() < self.cf_ratio and batch_unsup is not None: # 无条件 model
        # if np.random.rand() < 0.2 and batch_unsup is not None:
            self.forward_backward(batch_unsup, cond_unsup, unsup=True)  # 2成
        else:
            self.forward_backward(batch, cond) # 有条件的 model 
    ...
    def forward_backward(self, batch, cond , unsup=False): ### add flag
        zero_grad(self.model_params)
        # print("### cond 1", cond['input_ids'].shape)
        # print("### cond 2 ", cond['input_mask'].shape)
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_cond = {
                k: v[i : i + self.microbatch].to(dist_util.dev())
            ...
            last_batch = (i + self.microbatch) >= batch.shape[0]
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())
            # print("### micro_cond 1", micro_cond['input_ids'].shape)
            # print("### micro_cond 2 ", micro_cond['input_mask'].shape)
            
            ######################### mix ######################### 
            # 条件模型，且有一定比例1成更新
            if np.random.rand() < self.cf_ratio and not unsup: # batch code
            ...
            # 五 条件模型    
            if unsup: # each one in batch code
            ...
            if last_batch or not self.use_ddp:
                losses = compute_losses() # here, 返回一个字典，含loss这个键
            else:
                with self.ddp_model.no_sync():
                    losses = compute_losses()
