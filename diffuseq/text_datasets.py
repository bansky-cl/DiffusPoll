...
def load_data_text(
    ...
    loaded_vocab=None,
    loop=True,
    dataset='wbp-all-256', ### need controls, you can resize it
):
    """
    ...
    if split != 'test':
        ...
        data_loader = DataLoader(
            ...
            shuffle=not deterministic,
            num_workers=4,
            # num_workers=2, # shm is unsufficient
        )
...
###  find all sep token ids & return a list
def get_all_sep_index(lst=None, item=102): # [sep] in bert tokenizer
...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    tokenized_datasets = raw_datasets.map(
        tokenize_function,
        batched=True,
        num_proc=vocab_dict.worker, # bert 4 to t5  1
    ...
    def merge_and_mask(group_lst):
        ...
        for i in range(len(group_lst['input_id_x'])):
            ...
            ### add sep token 
            current_sep_token_ids = get_all_sep_index(src + [vocab_dict.sep_token_id] + trg, item=vocab_dict.sep_token_id)
            # print("$$$\n", src + [vocab_dict.sep_token_id] + trg) #debug
            attn_mask.append(current_sep_token_ids)
        
        ###
        group_lst['attn_mask'] = attn_mask
...
def get_corpus(data_args, dataset, seq_len, split='train', loaded_vocab=None): # add para
    ...
    if split == 'train':
        print(f'### Loading form the {dataset}_TRAIN set...')
        path = f'{data_args.data_dir}/{dataset}_train.jsonl' # wbp_train.jsonl and weibo_train.jsonl
    elif split == 'valid':
        print(f'### Loading form the {dataset}_VALID set...')
        path = f'{data_args.data_dir}/{dataset}_valid.jsonl'
    elif split == 'test':
        print(f'### Loading form the {dataset}_TEST set...')
    ...
    # get tokenizer.
    vocab_dict = loaded_vocab # mytokenizer 类
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with torch.no_grad():
            ...
            ###
            out_kwargs['attn_mask'] = np.array(self.text_datasets['train'][idx]['attn_mask'])
